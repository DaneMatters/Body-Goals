...
const PROGRAM_LIBRARY_VERSION=1;
...
function scheduleStreak(){return dayStreak(iso=>(state.scheduleLog||[]).some(l=>l.date===iso))}
...
function fullHouseToday(){
  const iso=isoDate(nowDate());
  if(!(state.schedule||[]).length)return false;
  const allSched=state.schedule.every(item=>(state.scheduleLog||[]).some(l=>l.scheduleId===item.id&&l.date===iso));
  if(!allSched)return false;
  const dow=new Date(iso+'T00:00:00').getDay();
  const workoutDone=!state.program[dow]||state.workouts.some(w=>w.date===iso);
  const insanityDue=dow>=1&&dow<=5;
...
const BADGE_DEFS=[
  {id:'plates225',name:'Two Plates',glyph:'225',hint:'A main lift (bench, squat, or deadlift) hits a 225 lb estimated 1RM',check:()=>mainLiftBestSince()>=225},
  {id:'plates315',name:'Three Plates',glyph:'315',hint:'A main lift hits a 315 lb estimated 1RM',check:()=>mainLiftBestSince()>=315},
  {id:'plates405',name:'Four Plates',glyph:'405',hint:'A main lift hits a 405 lb estimated 1RM',check:()=>mainLiftBestSince()>=405},
  {id:'session10k',name:'10K Session',glyph:'10K',hint:'A single workout hits 10,000 lb of volume',check:()=>myWorkoutsSince().some(w=>workoutVolume(w)>=10000)},
  {id:'vol250k',name:'Quarter Million',glyph:'¼M',hint:'250,000 lb lifted in total',check:()=>myWorkoutsSince().reduce((n,w)=>n+workoutVolume(w),0)>=250000},
  {id:'vol500k',name:'Half Million',glyph:'½M',hint:'500,000 lb lifted in total',check:()=>myWorkoutsSince().reduce((n,w)=>n+workoutVolume(w),0)>=500000},
  {id:'vol1m',name:'Million Pound Club',glyph:'1M',hint:'1,000,000 lb lifted in total',check:()=>myWorkoutsSince().reduce((n,w)=>n+workoutVolume(w),0)>=1000000},
  {id:'firstblood',name:'First Blood',glyph:'PR',hint:'Your first logged PR',check:()=>myWorkoutsSince().some(w=>(w.exercises||[]).some(e=>e.sets.some(s=>s.pr)))},
  {id:'hattrick',name:'Hat Trick',glyph:'3×',hint:'A PR on bench, squat, and deadlift in the same week',check:hatTrickSince},
  {id:'streak7',name:'7-Day Streak',glyph:'7',hint:'A schedule item marked done 7 days in a row',check:()=>scheduleStreak()>=7},
  {id:'streak30',name:'30-Day Streak',glyph:'30',hint:'A schedule item marked done 30 days in a row',check:()=>scheduleStreak()>=30},
  {id:'comeback',name:'The Comeback',glyph:'↺',hint:'Reopen a finished workout and complete it',check:()=>myWorkoutsSince().some(w=>w.reopened)},
  {id:'freelancer',name:'Freelancer',glyph:'+1',hint:'Log an exercise outside the plan with + ADD EXERCISE',check:()=>myWorkoutsSince().some(w=>(w.exercises||[]).some(e=>e.added))},
  {id:'ironwill',name:'Iron Will',glyph:'10',hint:'Finish 10 workouts with every set checked off',check:()=>myWorkoutsSince().filter(w=>w.allSetsDone).length>=10},
  {id:'hydrated',name:'Hydrated',glyph:'H₂O',hint:'Hit your 3.5 L water target 7 days in a row',check:()=>waterStreak()>=7},
  {id:'dialedin',name:'Dialed In',glyph:'P',hint:'Hit your 170 g protein target 7 days in a row',check:()=>proteinStreak()>=7},
  {id:'fullhouse',name:'Full House',glyph:'★',hint:'Every scheduled item done in a single day',check:fullHouseToday},
  {id:'phase1',name:'Phase 1 Complete',glyph:'I',hint:'Weeks 1–4 of the 12-week program finished',check:()=>weeksIntoProgram()>=4},
  {id:'phase2',name:'Phase 2 Complete',glyph:'II',hint:'Weeks 5–8 of the 12-week program finished',check:()=>weeksIntoProgram()>=8},
  {id:'phase3',name:'Phase 3 Complete',glyph:'III',hint:'The full 12-week program finished',check:()=>weeksIntoProgram()>=12},
  {id:'century',name:'Century',glyph:'100',hint:'100 workouts logged',check:()=>myWorkoutsSince().length>=100},
];
...
let ui={page:lastPage(),historyFilter:'all',session:state.activeWorkout||null,restEnds:0,restTimer:null,modal:null,editingProgram:false,editingSchedule:false,editingFavorites:false,nowDetailFor:null,historyDate:'',volumeRange:'3m',e1rmExercise:'',e1rmRange:'6m',expandedSets:new Set(),progressTab:'overview'};
let homeTimer=null;
function defaults(){return {workouts:[],food:[],weights:[],activeWorkout:null,settings:{weightStep:5,restDefault:90},insanity:[],program:deep(DEFAULT_PROGRAM),schedule:[],scheduleLog:[],scheduleVersion:0,programVersion:0,stretchVideoVersion:0,water:[],favorites:[],xp:0,badges:[],gamifyStart:isoDate(nowDate())}}
...
function addNewProgramModal(){
  ...
  const id='p'+uid();
  saveActiveProgramToLibrary();
  state.programLibrary[id]={label,program:{},startDate:isoDate(nowDate())};
  state.program={};
...
function markScheduleDone(id,opts){opts=opts||{};const iso=isoDate(nowDate());state.scheduleLog=state.scheduleLog||[];const existing=state.scheduleLog.find(l=>l.scheduleId===id&&l.date===iso);if(existing){unmarkScheduleLog(existing.id);return}state.scheduleLog.push({id:uid(),scheduleId:id,date:iso,ts:Date.now()});const item=(state.schedule||[]).find(s=>s.id===id);if(item&&item.type==='water'&&!opts.skipWaterLog)logWater(500,{skipScheduleMark:true});save();awardXP(5);render()}
function markNextScheduleOfType(type,opts){const iso=isoDate(nowDate());const item=(state.schedule||[]).find(s=>s.type===type&&!(state.scheduleLog||[]).some(l=>l.scheduleId===s.id&&l.date===iso));if(item)markScheduleDone(item.id,opts)}
function waterToday(){const iso=isoDate(nowDate());return (state.water||[]).filter(w=>w.date===iso).reduce((a,b)=>a+(+b.amountMl||0),0)}
function logWater(ml,opts){opts=opts||{};const t=new Date();state.water=state.water||[];state.water.push({id:uid(),date:isoDate(t),time:t.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}),amountMl:ml});if(!opts.skipScheduleMark)markNextScheduleOfType('water',{skipWaterLog:true});save()}
...
function homePage(){const d=nowDate();const todayFoods=state.food.filter(f=>f.date===isoDate(d));const c=todayFoods.reduce((a,b)=>a+(+b.calories||0),0),pr=todayFoods.reduce((a,b)=>a+(+b.protein||0),0);return `<div class="page"><div class="muted" style="text-align:center;margin-bottom:14px;font-size:14px">${fmtNowHeader(d)}</div>${levelLineHTML(true)}<div class="card now-card"><div class="row between"><div class="eyebrow">UP NOW${scheduleShiftBadge()}</div><button class="btn small ${ui.editingSchedule?'primary':'ghost'}" data-toggle-edit-schedule>${ui.editingSchedule?'DONE':'EDIT SCHEDULE'}</button></div>${ui.editingSchedule?scheduleEditHTML():currentBlockHTML()}</div>${ui.editingSchedule?'':comingUpHTML()}<div class="metric-grid"><div class="metric"><div class="subtle">CALORIES LOGGED</div><div class="val">${Math.round(c)}</div></div><div class="metric"><div class="subtle">PROTEIN LOGGED</div><div class="val">${Math.round(pr)} g</div></div><div class="metric"><div class="subtle">WATER LOGGED</div><div class="val">${(waterToday()/1000).toFixed(1)} / 3.5 L</div></div></div></div>`}
...
function insanityCatchupHTML(){const m=missedInsanity();return m.length?`<div class="muted" style="margin-top:14px">${m.length} past Insanity day${m.length===1?'':'s'} not logged <button class="btn small primary" data-insanity-catchup>CATCH UP</button></div>`:''}
...
function programProgressHTML(){
  ...
  return `<div class="level-line" style="margin-bottom:14px"><div class="row between"><span>WEEK ${week} of ${totalWeeks}${phaseLabel}</span><span class="subtle">${pct}%</span></div><div class="xp-track"><div class="xp-fill" style="width:${pct}%"></div></div></div>`;
}
function programLibraryHTML(){
  ...
  return `<div class="card"><div class="row between"><div class="section-title">PROGRAMS</div><button class="btn small ghost" data-add-program>+ NEW</button></div><div class="muted" style="margin-bottom:10px">Switch between programs, or build your own from scratch.</div>${rows||'<div class="history-empty">No saved programs yet.</div>'}</div>`;
}
function programViewHTML(d){const days=progDays();const phaseIdx=currentPhaseIdx();return `${programProgressHTML()}<div class="muted" style="margin-bottom:8px">Tap a day to see it, or EDIT to add, remove or change your workout days.</div>${days.length?days.map(day=>{const x=state.program[day];return `<details class="program-day" data-day="${day}" ${day===d.getDay()?'open':''}><summary><span><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${esc(x.color||'#4aa3ff')};margin-right:8px"></span>${DAYN[day]} • ${esc(x.name)}</span><span>⌄</span></summary>${x.exercises.length?x.exercises.map(e=>{const min=e.phased?e.reps[phaseIdx]:e.monthReps?e.monthReps[phaseIdx]:e.min,max=e.phased?e.reps[phaseIdx]:e.monthReps?e.monthReps[phaseIdx]:e.max,pctLabel=e.phased?` @ ${Math.round(e.pct[phaseIdx]*100)}%`:'',tag=e.phased?' <span class="badge">PHASED</span>':e.monthReps?' <span class="badge">MONTH '+(phaseIdx+1)+'</span>':'';return `<div class="exercise-preview row between"><span>${esc(e.name)}${tag}</span><b>${e.sets} × ${min===max?min:`${min}-${max}`}${pctLabel}</b></div>`}).join(''):'<div class="exercise-preview muted">No exercises yet — tap EDIT to add some.</div>'}${x.exercises.length?`<button class="btn small ghost full" style="margin-top:10px" data-start-day="${day}">${state.activeWorkout?'RESUME WORKOUT':'START / LOG THIS WORKOUT'}</button>`:''}</details>`}).join(''):'<div class="history-empty">No workout days yet. Tap EDIT to build your program.</div>'}`}
