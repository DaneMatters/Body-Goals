...
function workoutsPage(){const d=nowDate(),insanityToday=activeProgramInsanityEnabled()&&d.getDay()>=1&&d.getDay()<=6;const loggedThisWeek=state.workouts.filter(w=>{const diff=(d-new Date(w.date+'T12:00:00'))/86400000;return diff>=-1&&diff<7}).length;const progress=programProgressHTML();return `<div class="page"><img src="me-gym.jpg" class="workout-banner"><div class="card"><div class="row between"><div class="section-title">THIS WEEK</div><div class="muted">${loggedThisWeek} strength day${loggedThisWeek===1?'':'s'} logged</div></div>${weekHTML()}${progress?`<div style="margin-top:14px">${progress}</div>`:''}</div>${insanityToday?`<div class="card"><div class="row between"><div class="muted">7:30 PM Insanity: <b>${esc(insanityDueFor(d)||'planned')}</b></div>${state.insanity.some(x=>x.date===isoDate(d))?`<span class="badge">DONE</span> <button class="mini-toggle" data-insanity-undo="${isoDate(d)}">Undo</button>`:'<button class="btn small primary" data-log-insanity>⚡ LOG</button>'}</div>${insanityCatchupHTML()}</div>`:''}${ui.editingProgram?'':selectedDayCardHTML(d)}<div class="card"><div class="row between gap wrap"><div class="section-title">YOUR PROGRAM</div><div class="row gap" style="flex-wrap:wrap">${programSwitchSelectHTML()}${ui.editingProgram?`<button class="btn small primary" data-toggle-edit-program>DONE</button>`:''}</div></div>${ui.editingProgram?programEditHTML():programViewHTML()}</div>${historySection()}</div>`}
...
function programViewHTML(){const lib=state.programLibrary[state.activeProgramId];return `<div class="big">${esc(lib.label)}</div>`}
