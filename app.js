...
function fullHouseToday(){
  ...
  const insanityDue=activeProgramInsanityEnabled()&&dow>=1&&dow<=6;
...
let ui={page:lastPage(),historyFilter:'all',session:state.activeWorkout||null,restEnds:0,restTimer:null,modal:null,editingProgram:false,editingSchedule:false,editingFavorites:false,nowDetailFor:null,historyDate:'',volumeRange:'3m',e1rmExercise:'',e1rmRange:'6m',expandedSets:new Set(),progressTab:'overview',workoutsDay:null};
let homeTimer=null;
...
function todaysEvents(){
  ...
  if(activeProgramInsanityEnabled()&&d.getDay()>=1&&d.getDay()<=6){const baseMin=parseTimeStr('7:30 PM'),min=baseMin+offset;items.push({id:'insanity',name:insanityDueFor(d)||'Insanity Cardio',time:fmtTime12(min),min,done:state.insanity.some(x=>x.date===iso)})}
...
function workoutsPage(){const d=nowDate(),p=state.program[d.getDay()],insanityToday=activeProgramInsanityEnabled()&&d.getDay()>=1&&d.getDay()<=6;const loggedThisWeek=state.workouts.filter(w=>{const diff=(d-new Date(w.date+'T12:00:00'))/86400000;return diff>=-1&&diff<7}).length;const todayWorkout=state.workouts.find(w=>w.date===isoDate(d));return `<div class="page"><img src="me-gym.jpg" class="workout-banner"><div class="card"><div class="row between"><div><div class="eyebrow">TODAY'S TRAINING</div><div class="big">${p?esc(p.name)+' • '+esc(p.time||''):'Recovery Day'}</div><div class="muted" style="margin-top:8px">${p?'Body Shop Fitness, Warsaw, Indiana • 45 min':'No mandatory morning training'}</div></div><div class="status-pill ${todayWorkout?'done':''}">${todayWorkout?'DONE':'PLANNED'}</div></div>${p?(state.activeWorkout?`<button class="btn primary full" style="margin-top:20px" data-start>RESUME WORKOUT</button>`:todayWorkout?`<button class="btn ghost full" style="margin-top:20px" data-history-id="${todayWorkout.id}">VIEW TODAY'S WORKOUT</button>`:`<button class="btn primary full" style="margin-top:20px" data-start>START / LOG WORKOUT</button>`):''}${insanityToday?`<div class="muted" style="margin-top:16px">7:30 PM Insanity: <b>${esc(insanityDueFor(d)||'planned')}</b> ${state.insanity.some(x=>x.date===isoDate(d))?`<span class="badge">DONE</span> <button class="mini-toggle" data-insanity-undo="${isoDate(d)}">Undo</button>`:'<button class="btn small primary" data-log-insanity>LOG</button>'}</div>`:''}${insanityCatchupHTML()}</div><div class="card"><div class="row between"><div class="section-title">THIS WEEK</div><div class="muted">${loggedThisWeek} strength day${loggedThisWeek===1?'':'s'} logged</div></div>${weekHTML()}</div>${ui.editingProgram?'':selectedDayCardHTML(d)}<div class="card"><div class="row between gap wrap"><div class="section-title">YOUR PROGRAM</div><div class="row gap" style="flex-wrap:wrap">${programSwitchSelectHTML()}${ui.editingProgram?`<button class="btn small primary" data-toggle-edit-program>DONE</button>`:''}</div></div>${ui.editingProgram?programEditHTML():programViewHTML()}</div>${historySection()}</div>`}
...
function programViewHTML(){const lib=state.programLibrary[state.activeProgramId];return `<div class="big" style="margin-bottom:10px">${esc(lib.label)}</div>${programProgressHTML()}`}
