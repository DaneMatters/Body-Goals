...
let ui={page:lastPage(),historyFilter:'all',session:state.activeWorkout||null,restEnds:0,restTimer:null,modal:null,editingProgram:false,editingFavorites:false,historyDate:'',volumeRange:'3m',e1rmExercise:'',e1rmRange:'6m',expandedSets:new Set(),progressTab:'overview',workoutsDay:null,historyCalMonth:''};
let homeTimer=null;
...
function homePage(){
  ...
  return `<div class="page">
    ...
    ${levelLineHTML(true)}
    <div class="card"><div class="section-title">CURRENT PRs</div><div class="metric-grid">${prs.map(p=>`<div class="metric"><div class="subtle">🏆 ${esc(p.short)}</div><div class="val">${p.best?Math.round(p.best.y).toLocaleString()+' lb':'—'}</div>${p.best?`<div class="subtle" style="margin-top:2px">${esc(p.best.date)}</div>`:''}</div>`).join('')}</div></div>
...
function shiftCalMonth(dir){const key=ui.historyCalMonth||calMonthKey(nowDate());const [y,m]=key.split('-').map(Number);const d=new Date(y,m-1+dir,1);ui.historyCalMonth=calMonthKey(d)}
function historyCalendarHTML(){
  ...
  return `<div class="row between" style="margin-bottom:10px"><button class="btn small ghost" data-cal-nav="-1">‹</button><div class="section-title">${esc(monthLabel)}</div><button class="btn small ghost" data-cal-nav="1">›</button></div><div class="cal-weekdays">${['S','M','T','W','T','F','S'].map(d=>`<div>${d}</div>`).join('')}</div><div class="cal-grid">${cells.join('')}</div>`;
}
