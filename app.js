...
function programProgressHTML(){
  ...
  const lib=state.programLibrary[state.activeProgramId];
  return `<div class="level-line" style="margin-bottom:14px">${lib?`<div class="big" style="margin-bottom:6px">${esc(lib.label)}</div>`:''}<div class="row between"><span>WEEK ${week} of ${totalWeeks}${phaseLabel}</span><span class="subtle">${pct}%</span></div><div class="xp-track"><div class="xp-fill" style="width:${pct}%"></div></div></div>`;
}
