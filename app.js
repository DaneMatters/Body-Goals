...
let badgeSvgSeq=0;
function badgeIconSVG(glyph,earned){
  ...
  const stroke=earned?'#8a6a1c':'#333';
  ...
  return `<svg viewBox="0 0 64 64" class="badge-icon" aria-hidden="true"><defs><radialGradient id="${uidS}" cx="38%" cy="30%" r="75%"><stop offset="0%" stop-color="#fff3c4"/><stop offset="35%" stop-color="#ffd76f"/><stop offset="75%" stop-color="#c98f1e"/><stop offset="100%" stop-color="#8a5e12"/></radialGradient><filter id="${uidS}g" x="-40%" y="-40%" width="180%" height="180%"><feDropShadow dx="0" dy="0" stdDeviation="2.2" flood-color="#ffd76f" flood-opacity="0.55"/></filter></defs>${glow}<text x="32" y="37" text-anchor="middle" font-size="${glyph.length>3?14:18}" font-weight="900" fill="${earned?'#241a02':'#666'}">${esc(glyph)}</text></svg>`;
}
...
function programSwitchSelectHTML(){
  ...
  return `<select class="field" style="width:auto;padding:8px 26px 8px 10px;font-size:13px;border-radius:12px" data-program-select>${Object.entries(lib).map(([id,p])=>`<option value="${esc(id)}" ${id===state.activeProgramId?'selected':''}>${esc(p.label)}</option>`).join('')}</select>`;
}
